...
pub fn register_shaders(compiler: &mut SlangCompiler) {
    ...
    // Mirror the cpic, node_particle_lists and pml cargo features into shader macros so the
    // shader Node/particle layouts and sort kernels match GpuGridNode, Kinematics,
    // ParticleProperties and GridArgs on the Rust side. slosh sets these itself (unlike DIM,
    // which the consumer picks) since they're crate-layout invariants.
    ...
    compiler.set_global_macro("SLOSH_PML", if cfg!(feature = "pml") { 1 } else { 0 });
}
